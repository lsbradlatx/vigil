# Get Vigil Live on the Internet — Complete Beginner Guide
...
You’ll need:
- Your project folder: `vigil` (on your Desktop).
...
3. **Repository name:** type `vigil` (or any name you like — no spaces).
...
`https://github.com/YOUR_USERNAME/vigil.git`.  
...
1. Open **File Explorer** and go to your Desktop, then into the **vigil** folder.
...
- Open PowerShell from the Start menu, then type:  
  `cd C:\Users\bradl\OneDrive\Desktop\vigil`  
...
Run these commands **in order**, one at a time, in PowerShell (still in the `vigil` folder).
...
```
git commit -m "Initial commit - Vigil app"
```  
Meaning: “Save this snapshot and call it ‘Initial commit - Vigil app’.”

**Command 4 — Name the GitHub repo (use YOUR GitHub URL):**  
You need to replace `YOUR_USERNAME` with your real GitHub username and `vigil` if you used a different repo name.

```
git remote add origin https://github.com/YOUR_USERNAME/vigil.git
```  
...
**You’re done with Part 1.** If you go to `https://github.com/YOUR_USERNAME/vigil` in your browser, you should see all your project files there.
...
In PowerShell, in your **vigil** folder, run:
...
6. You should see a list of your repositories. Click **vigil** (or whatever you named it). If you don’t see it, click **Configure GitHub App** and give Railway access to that repo, then try again.
...
1. Click back on your **app service** (the one that says “vigil” or your repo name, not Postgres).
...
3. Click **Generate domain** (or **Add domain** and use the default). Railway will create a URL like:
   `https://vigil-production-xxxx.up.railway.app`
